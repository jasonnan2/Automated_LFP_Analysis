function plotPowerByCondition(powerdata,f, freqRangeAll, freq_list, color, saveName, trialTypes, conditionOrder, chans2plot, sortedChans)
    for freq=1:length(freq_list)
        ...
        for a=1:aMax
            ...
            combinations=nchoosek(1:length(trialTypes),2);
            
            % size rat x region x dose x freqPower x trial type
            for tri=1:size(powerdata,5)
                subplot(size(powerdata,5),1,tri)
                ylabel(trialTypes{tri})
                hold on
                data = nanmean(squeeze(powerdata(:,chans,:,locs(1):locs(2),tri)),4); % get all rat, all dose
